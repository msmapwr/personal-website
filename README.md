...
页面路由使用按需加载：首页、项目、技能、博客和详情页会在进入对应路由时加载，避免所有页面代码同时进入首页关键路径。加载期间显示稳定的占位区域，减少布局跳动。
...
项目图片没有放在 XML 中，因为图片不需要翻译。项目 ID 与图片路径的对应关系集中在 `src/content/projectImages.ts`。当前页面通过 `ResponsiveImage` 按 AVIF → WebP → PNG 顺序加载，并根据视口使用 640/1024 宽度资源；PNG 原图仍保留，方便兼容、重新压缩或后续生成其他尺寸。图片组件同时用于项目卡片和详情页，避免两处加载策略不一致。
